...
2. **Fork 仓库**（推荐）：在 [github.com/austinxyz/wealth-llm-wiki](https://github.com/austinxyz/wealth-llm-wiki) 右上角点 Fork，把仓库 fork 到你自己的 GitHub 账号下
   - Fork 的好处：你可以自由添加个人资料、自定义内容，又能随时从上游 pull 更新
   - 不 fork 也行：直接 Clone，但以后你的修改和上游更新会有冲突
3. 克隆你 fork 后的仓库：
   ```
   git clone https://github.com/你的用户名/wealth-llm-wiki
   cd wealth-llm-wiki
   ```
   （不会 git？GitHub 页面 Code → Download ZIP 解压也行，但以后更新要重下）
4. 启动：`claude`
5. 试一下：输入 `/wealth-freshness`，看知识库时效扫描报告
...
4. `git commit + git push`（推送到你自己 fork 的仓库）
5. 想回馈社区？在 GitHub 对上游仓库提 PR，组长 review 后合并

**维护流（定期 audit + refresh）**
1. `/wealth-freshness` — 扫描全库，列出过期或需更新的条目
2. 对标记为过期的条目逐一运行 `/wealth-audit <文件>` — 联网核对官方数据，给出修改建议
3. 确认后 `/wealth-sync <wiki条目>` — 将最新数据同步进 wiki
4. `git commit + git push` 保存更新
5. 建议每季度跑一次，税季前必跑（每年 1–3 月税务数字变化最多）
